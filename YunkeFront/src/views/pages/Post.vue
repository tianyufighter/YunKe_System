                        ...
                        <div>
                            ...
                            <div class="comments-container mt-4">
                                ...
                                <span class="flex justify-end" v-if="post.commentList.length > post.len">
                                    <a class="inline-flex items-center text-sm" href="javascript:;" @click="post.len = post.commentList.length"><span>查看全部</span> <feather-icon icon="ChevronsRightIcon" svgClasses="h-4 w-4"></feather-icon></a>
                                </span>
                            </div>
                            <!--发表评论的输入框-->
        ...
        <div class="vx-row">
            <div class="vx-col w-full lg:w-3/4">
                <div class="flex justify-center mt-base">
                    <vs-button id="button-load-more-posts" class="vs-con-loading__container" @click="loadContent" v-if="userPosts.length < totalPost">加载更多</vs-button>
                </div>
            </div>
        </div>
    </div>
    ...
    closeSidebar() {
      ...
      getPostByPage({
        ...
        for (let i = 0; i < res.data.data.list.length; i++) {
          // 设置显示评论的条数
          res.data.data.list[i].len = 2;
          this.userPosts.push(res.data.data.list[i])
        }
      }).catch(err => {
        console.log("err = ", err)
      })
