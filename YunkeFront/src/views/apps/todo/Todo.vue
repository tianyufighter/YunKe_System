...
<template>
    <div id="todo-app" class="border border-solid d-theme-border-grey-light rounded relative overflow-hidden">
      ...
      <div :class="{'sidebar-spacer': clickNotClose}" class="app-fixed-height border border-r-0 border-b-0 border-t-0 border-solid d-theme-border-grey-light app-fixed-height">
        ...
        <VuePerfectScrollbar class="todo-content-scroll-area" :settings="settings" ref="todoListPS">
            <transition-group class="todo-list" name="list-enter-up" tag="ul" appear>
                <li class="cursor-pointer todo_todo-item" v-for="(todo, index) in filterTodo" :key="String(todoFilter) + String(todo.id)" :style="[{transitionDelay: (index * 0.1) + 's'}]">
                    <todo-item :todoItem="todo" @showDisplayPrompt="showDisplayPrompt($event)"></todo-item>
                </li>
            </transition-group>
        </VuePerfectScrollbar>
          <!-- /TODO LIST -->
      ...
      <!-- EDIT TODO DIALOG -->
      <todo-edit :displayPrompt="displayPrompt" @shijian="hah" :todoItem="todoItem" @hideDisplayPrompt="hidePrompt" v-if="displayPrompt"></todo-edit>
    </div>
</template>
