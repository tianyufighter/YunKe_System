...
<template>
    <vs-prompt
        vs-title="编辑任务"
        :vs-accept-text= "submitOrRestore"
        vs-cancel-text = "移除"
            ...
            <form>
                <div class="vx-row">
                    <div class="vx-col w-1/6 self-center">
                        <vs-checkbox v-show="!isTrashedLocal" v-model="isDoneLocal" class="w-8"></vs-checkbox>
                    </div>
...
<script>
import {doAddOrUpdateTask, doDeleteTask} from "../../../network";
...
export default {
    ...
    data() {
        return {
            ...
            isTrashedLocal: this.todoItem.isTrashed,
    ...
    computed: {
        ...
        submitOrRestore () {
          return this.isTrashedLocal == true ? '恢复' : '提交'
        },
        validateForm() {
    ...
    methods: {
        ...
        submitTodo() {
          if (this.isTrashedLocal == true) {
            this.isTrashedLocal = false;
          }
          let taskObj = {};
          ...
          taskObj.isTrashed = this.isTrashedLocal;
          taskObj.tagNameList = this.tagsLocal;
          doAddOrUpdateTask(taskObj).then(res => {
            if (res.data.code == 200) {
              ...
              this.todoItem.isTrashed = this.isTrashedLocal;
